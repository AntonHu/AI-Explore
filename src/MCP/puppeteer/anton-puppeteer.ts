...
const consoleLogs: string[] = []; // 存储浏览器控制台日志
const screenshots = new Map<string, string>(); // 存储屏幕截图，键为名称，值为 base64 编码的图像数据
...
async function ensureBrowser() {
  if (!browser) {
    ...
    browser = await puppeteer.launch(
      process.env.DOCKER_CONTAINER ? docker_args : npx_args // 根据环境变量选择不同的参数
    );
  ...
  switch (name) {
    ...
    case "puppeteer_screenshot": {
      ...
      const screenshotPath = path.join(
        __dirname,
        "screenshots",
        `${args.name}.png` // 截图保存路径
      );
      console.log("screenshotPath:", screenshotPath);
      fs.writeFileSync(
        screenshotPath,
        Buffer.from(screenshot as string, "base64") // 将截图保存到文件
      );

      screenshots.set(args.name, screenshot as string); // 将截图添加到 screenshots Map
      server.notification({
        // 发送通知，告知资源列表已更改
